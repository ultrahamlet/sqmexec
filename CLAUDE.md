...
| シーンに書いた箱が 2 倍の大きさで出る / 他のパーツを飲み込む | `(box ..(size w h d))` の `size` は**半径 (half-extent)**。全長ではない | 半分の値を書く |
| 出力 PNG が作られないのに `sqm` は成功して見える | カメラが**真上など完全に垂直**だと縮退し、bv が空のまま終了する。**前回の PNG が残るので「同じ絵が出た」ように見える** | 視線をわずかに傾ける。出力の更新時刻を確認する |
| 透明な材質に色が着かない | この build の透過は**無色固定**。`(color ..)` も `tintglass` の `absorb-*` も効かない | `kt` を絞って自分の色を出す (低いほど濃い)。詳細は `scenes/glass_apple.ssq` の冒頭 |
