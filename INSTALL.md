...
## 3. sdfmodeler (エディタ) を起動する

> **Chrome を全部閉じてから**始めてください (理由は下記)。
...
ローカルサーバが `http://localhost:8642` で立ち、**Chrome が
`--use-angle=vulkan` 付きで開きます**。フラグは `serve.ps1` が自動で付けるので、
指定する必要はありません。

### なぜ Vulkan を指定するのか

sdfmodeler は形を編集するたびに WebGL のシェーダーを組み立て直します。
Windows Chrome の既定は **ANGLE → HLSL → FXC (D3D11)** で、この FXC が
sdfmodeler の生成する GLSL を苦手にしています。**25 ノードあたりを境に**、
2 つの壊れ方をします (RTX 3090 で実測):

| ノード数 | コンパイル | 描画 |
|---|---|---|
| 8 〜 15 | 1 秒前後 | 正常 |
| **25** | **22 〜 69 秒** | **全画素が同じ色 (破綻)** |

厄介なのは 2 つ目です。**シェーダーのリンクは「成功」を返すのに、絵だけが
壊れます。**エラーもコンソール警告も出ないので、モデルの作り方を間違えたのか、
エディタが壊れたのか、区別がつきません。

Vulkan バックエンドは HLSL を経由せず SPIR-V を吐くため、**この 2 つとも
起きません** (2026-08-26 に実測確認)。GLSL 側には問題がなく、原因は FXC の
経路そのものでした。

> **Chrome が既に起動していると、フラグは黙って無視されます。**
> 2 つ目の Chrome プロセスは URL を既存のプロセスに渡して終了するだけなので、
> 新しいフラグは効きません。この場合、上の表の症状をそのまま踏みます。
...
- `..., Direct3D11 vs_5_0 ps_5_0, D3D11)` → **効いていません**。
  まず Chrome が既に起動していなかったか疑ってください (最も多い原因)。
  それでも D3D11 なら `chrome://gpu` の先頭に理由が出ています
...
`serve.ps1` は Chrome が入っていれば `--use-angle=vulkan` 付きで自動的に開きます
(`-NoBrowser` で抑止)。**素の `python serve.py` を直に叩くとこの経路を外す**ので、
Windows ではラッパを使ってください。
...
| エディタの編集がカクつく・数十秒固まる | FXC のシェーダー再コンパイル | Vulkan バックエンドになっているか上の方法で確認 |
| **形が消えて画面が単色になる** | 同上 (FXC の無音の誤コンパイル) | 同上。**モデルの作り方の問題ではありません** |
